...
- **`saveSettings` (有給付与設定保存)**
  - 正常な日付（MM-DD）および日数の更新。
  - 日付フォーマット不正、有給日数範囲外（1〜40以外）、非数値入力時のエラーメッセージ設定とリダイレクト検証。
- **各種設定更新**
  - コピーライト表示、システム名表示、勤怠期間、バッチ処理、アラート閾値、CSVファイル名パターンなどの更新正常系、および `IllegalArgumentException` 等の例外発生時のエラーハンドリング検証。
...
#### `ProfileControllerTest` (プロフィール・勤務クラスコントローラー)
- **`showProfile` (プロフィール画面表示)**
  - ログイン中ユーザーのプロフィール、有給消化日数・有給残日数の計算検証。
  - 有給付与日数が `null` の際に `0` として安全に計算され表示されること。
  - DBアクセス例外等のエラー発生時のエラー画面表示検証。
- **`updateWorkScheduleClass` (勤務クラス更新)**
  - 選択した勤務クラス（勤務スケジュール）の更新正常系。
  - `IllegalArgumentException` (無効なクラス名) 発生時のエラー検証。
  - その他予期しない例外発生時のエラーハンドリング検証。
...
- お知らせの作成、公開期間の判定、ユーザー向け一覧取得ロジック等を検証。
...
- アクション実行時の監査ログの記録、検索条件に基づくログ抽出ロジック等を検証。
...
- 勤怠提出リマインドについて、当月分の通知が既に存在するユーザーへは重複作成しないこと（手動送信と自動ポーリングの重複対策）を検証。
...
- `UserMapperIntegrationTest` / `AttendanceSubmissionMapperIntegrationTest` / `UserNotificationMapperIntegrationTest`
