...
- **インラインスタイル禁止**: `style="..."` や `th:style` は原則使用しない。動的な状態は `th:classappend` で制御する。例外として、実行時計算値による寸法指定（例: プログレスバーの `width: N%`）のみ `th:style` を許容する。
- **カスタム `<style>` タグ禁止**: テンプレート内に `<style>` ブロックを書かない。スタイルは `src/main/resources/static/css/input.css` で定義する。
- **Bootstrap クラス禁止**: Bootstrap 由来のクラス・属性（`data-bs-*`、Bootstrap CSS 前提の `modal` 等）は使用しない。ただし `input.css` の `@layer components` で自前定義した `.btn` / `.alert` / `.badge` / `.form-input` 等の短縮コンポーネントクラス（テーマトークンを `@apply` 参照するもの）は正式パターンとして使用可。
